...
int16 ms = 100, envio_cont=0, borrame=0, velocidad_serial_save = 0, velocidad_serial = 100;
int16  velocidad_lcd = 200, velocidad_lcd_save = 0; 
int alarma_tem = 30, alarma_tem_save = 0;
int alarma_rh = 12, alarma_rh_save = 0, alarma_s1 = 50, alarma_s1_save = 0, alarma_s2 = 60, alarma_s2_save = 0;
...
void sti_TIMER0(){
   ...
   if(envio_cont == (ms/100)){
      borrame++;
      printf("\r%lihola mundo",borrame);
      envio_cont = 0;
   }*/
   ...
   if(flag_enviar_tem){
      ...
      printf("\f%d%d.%d%d %c",dato[5],dato[6],dato[7],dato[8],37);
   }
   if(flag_enviar_rh){
      lcd_gotoxy(1,1);
      printf(lcd_putc,"%d%d.%d%d C",dato[1],dato[2],dato[3],dato[4]);
      printf("\f%d%d.%d%d C",dato[1],dato[2],dato[3],dato[4]);
   }
   if(flag_enviar_sen1_2){
      lcd_gotoxy(1,1);
      printf(lcd_putc,"\f%li",sensor1);
      lcd_gotoxy(1,2);
      printf(lcd_putc,"\f%li",sensor2);
      /*printf("\f%li",sensor1);
      printf("\f%li",sensor2);
      */
   }
   printf("\f\r\r%d%d.%d%d %c",dato[5],dato[6],dato[7],dato[8],37);
